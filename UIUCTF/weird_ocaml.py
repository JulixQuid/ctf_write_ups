...
puzzle_v = """
    ...
    ('a81, 'flag_072, '    with open(f"{var}_sep.ml",'rw') as f:
        f.write(output_text)flag_073) p311_t *
    ...
    ('a89, 'a90, 'flag_    with open(f"{var}_sep.ml",'rw') as f:
        f.write(output_text)087, 'flag_099) p371_t *
    ...
    ('a100) p448_t *    with open(f"{var}_sep.ml",'rw') as f:
        f.write(output_text)
...
vars = puzzle_f.split(':')[1]
vars = vars.splitlines()
vars = [var.split('val_t')[0].strip() for var in vars if 'val_t' in var]

for var in vars[:1]:
    puzzle_v3 = '\n'.join([line for line in puzzle_v.splitlines() if var in line])
    output_list = [header,constraints,puzzle_f,puzzle_v3,end_]
    output_text = ''.join(output_list)
    with open(f"{var[1:]}_sep.ml",'w') as f:
        f.write(output_text)
